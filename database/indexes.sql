-- TaskManagerAPI Index Strategy
-- Optimized for common query patterns and performance
...
CREATE INDEX idx_categories_system_generated ON categories(is_system_generated);
...
CREATE INDEX idx_ai_logs_task_id ON ai_processing_logs(task_id);
...
CREATE INDEX idx_ai_logs_confidence ON ai_processing_logs(confidence_score DESC);
...
CREATE INDEX idx_rate_limits_ip_endpoint_window ON rate_limits(ip_address, endpoint, window_start);

-- Performance monitoring views
CREATE VIEW v_task_performance_stats AS
SELECT 
    u.username,
...
CREATE VIEW v_ai_processing_stats AS
SELECT 
    processing_type,
