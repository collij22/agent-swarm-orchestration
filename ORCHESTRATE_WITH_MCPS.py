#!/usr/bin/env python
"""
ORCHESTRATE WITH MCPS - Direct execution without I/O issues
This version directly runs the orchestrator with all fixes
"""
...
# CRITICAL: Keep original stdout/stderr references BEFORE any modifications
import io
_real_stdout = sys.stdout
_real_stderr = sys.stderr

# Create wrapper that never closes
class PersistentWrapper:
    ...
    def write(self, text):
        try:
            # Remove any Unicode that might cause issues
            if isinstance(text, str):
                # Replace problematic Unicode with ASCII
                text = text.encode('ascii', 'replace').decode('ascii')
            self.stream.write(text)
            self.stream.flush()
        except:
            # Fallback to direct OS write
...
sys.stdout = PersistentWrapper(_real_stdout)
sys.stderr = PersistentWrapper(_real_stderr)

# Override print to be bulletproof
import builtins
def bulletproof_print(*args, **kwargs):
...
builtins.print = bulletproof_print
...
class FakeProgress:
    ...
    def update(self, task_id, **kwargs):
        if task_id in self.tasks:
            print(f"[UPDATE] {self.tasks[task_id]}")
...
print("Starting Agent Swarm Orchestrator")
print("="*70)
