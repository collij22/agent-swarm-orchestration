#!/usr/bin/env python3
"""
Verify the intelligent orchestrator setup and run it
"""
...
def verify_async_methods():
    """Verify that all required async methods exist"""
    print("Verifying async methods...")
    ...
    try:
        ...
        print(f"  run_agent_async: {'[OK]' if has_async else '[MISSING]'}")
        print(f"  run_agent: {'[OK]' if has_sync else '[MISSING]'}")
        
        if not has_async and has_sync:
            print("\n[INFO] Only synchronous method available, will create async wrapper")
            return 'sync'
        elif has_async:
            print("\n[OK] Async method available")
            return 'async'
        else:
            print("\n[ERROR] No run_agent method found!")
            return None
            
    except Exception as e:
        print(f"\n[ERROR] Failed to verify methods: {e}")
        return None

def create_fixed_orchestrator():
    """Create a fixed version of the orchestrator that handles both sync and async"""
    print("\nCreating fixed orchestrator...")
...
'''
    ...
    fixed_path = Path("INTELLIGENT_ORCHESTRATOR_FIXED.py")
    fixed_path.write_text(fixed_code, encoding='utf-8')
    print(f"[OK] Created {fixed_path}")
    
    return fixed_path

def main():
    """Main verification and execution"""
    print("="*80)
    print("INTELLIGENT ORCHESTRATOR - Verification and Execution")
    print("="*80)
    print()
    ...
    method_type = verify_async_methods()
    
    if method_type is None:
        print("\n[ERROR] Cannot proceed without run_agent method")
        return 1
    
    # Create fixed orchestrator
    fixed_path = create_fixed_orchestrator()
    
    print("\n" + "="*80)
    print("RUNNING INTELLIGENT ORCHESTRATOR")
    print("="*80)
    print()
