    ...
    try:
        if HAS_RICH and console and hasattr(console, 'file') and console.file and not console.file.closed:
            if isinstance(message, (Panel, Table)):
                ...
                safe_print(f"[{style}]{message}[/{style}]")
            else:
                safe_print(message)
        else:
...
class EnhancedOrchestrator:
    ...
    async def execute_enhanced_workflow(
        ...
        if checkpoint_file and Path(checkpoint_file).exists():
            ...
            if checkpoint_file.startswith("checkpoint_"):
                ...
                if restoration:
                    ...
                    safe_print("[red]Failed to load enhanced checkpoint[/red]")
                    return False
                ...
                safe_print("[red]Failed to load checkpoint, starting fresh[/red]")
                return False
        else:
        ...
        try:
            ...
            if HAS_PHASE5 and self.mandatory_testing:
                ...
                if HAS_RICH:
                    safe_print(f"[green]Test report saved to: {test_report_path}[/green]")
        ...
        while True:
            ...
            if interactive and batch:
                if not Confirm.ask(f"Execute agents: {', '.join(batch)}?"):
                    safe_print("[yellow]Batch skipped[/yellow]")
                    for agent in batch:
    ...
    async def _execute_agent_with_enhanced_features(
        ...
        if self.enable_validation:
            ...
            if interactive and console:
                safe_print(f"[green]✅ Validation report saved to: {report_path}[/green]")
        if success and self.enable_validation:
...
async def main():
    ...
    try:
        ...
        if HAS_RICH:
            safe_print(f"[red]Error: {str(e)}[/red]")
        if args.verbose:
            import traceback
            traceback.print_exc()
        orchestrator.logger.close_session()
